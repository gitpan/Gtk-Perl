...
new (Class)
	SV *	Class
	CODE:
	RETVAL = GNOME_DRUID_PAGE_FINISH(gnome_druid_page_finish_new());
	OUTPUT:
	RETVAL
...
new_with_vals (Class, title, text, logo, watermark)
	...
	CODE:
	RETVAL = GNOME_DRUID_PAGE_FINISH(gnome_druid_page_finish_new_with_vals(title, text, logo, watermark));
	OUTPUT:
	RETVAL
